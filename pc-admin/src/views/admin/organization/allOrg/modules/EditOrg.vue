<template>
  <a-drawer title="编辑" :width="drawerWidth" :visible="visibleObj.editOrgVisible" :body-style="{ paddingBottom: '80px' }" @close="onCloseDrawer">
    <div class="table-page-search-wrapper">
      <a-form-model layout="inline" ref="ruleForm" :model="form" :rules="rules">
        <a-row :gutter="48">
          <a-col :xl="24">
            <a-form-model-item ref="name" label="机构名称" prop="name">
...
export default {
  ...
  watch: {
    lineObj: {
      handler(val) {
        if (this.visibleObj.editOrgVisible && val) {
          this.form = {
            name: val.name || '',
            account: val.account || '',
            tel: val.tel || '',
  ...
  methods: {
    onCloseDrawer() {
      this.visibleObj.editOrgVisible = false
      this.form = emptyForm()
    },
    handleClickSubmit() {
