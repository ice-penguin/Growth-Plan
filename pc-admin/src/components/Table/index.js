...
export default {
  ...
  methods: {
    ...
    loadData(pagination, filters, sorter) {
      ...
      // if (queryParam) {
      //   parameter = Object.assign(parameter, queryParam)
      // }
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          // 默认r为null的数据
          r = !r ? { data: [], pageNo: 1, pageSize: 10, totalCount: 0, totalPage: 0 } : r
          if (r) {
            this.localPagination =
              (this.showPagination &&
                Object.assign({}, this.localPagination, {
            ...
            if (this.localPagination && this.localPagination.total > 0) {
  ...
  render() {
    ...
    // 如果是外部表头需要隐藏（this.showHeader等于false）同时没有查询到数据（this.localDataSource.length === 0），则改变showHeader为false（不显示表头）
    if (this.localDataSource.length === 0 && !this.showHeader) {
